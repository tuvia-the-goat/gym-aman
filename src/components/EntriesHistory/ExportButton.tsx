...
const ExportButton = ({
  ...
  const getDepartmentName = (id: string, status?: string) => {
    if (status === "notAssociated") return "מחוץ ליחידה";
    const department = departments.find((dept) => dept._id === id);
    return department ? department.name : "-";
  };

  const getSubDepartmentName = (id: string, status?: string) => {
    if (status === "notAssociated") return "-";
    if (!id) return "-";
  ...
  const exportToExcel = async () => {
    try {
      ...
      if (startDate && endDate) {
        activeFilters.push(
          `טווח תאריכים: ${formatDate(startDate.toISOString())} עד ${formatDate(
            endDate.toISOString()
          )}`
        );
      } else if (startDate) {
      ...
      <body>
        <table>
          <tr class="title-row">
            <td colspan="${
              admin?.role === "generalAdmin" ? 8 : 7
            }">דוח כניסות - מערכת אימ"ון - ${formattedDate} &#128170;</td>
          </tr>
          <tr>
      ...
      allEntries.forEach((entry, index) => {
        ...
        tableHtml += `
          <tr class="${rowClass}">
            ...
            <td style="mso-number-format:'\\@'">${getDepartmentName(
              entry.departmentId,
              entry.status
            )}</td>
            <td style="mso-number-format:'\\@'">${getSubDepartmentName(
              entry.subDepartmentId || "",
              entry.status
            )}</td>
            ${
              admin?.role === "generalAdmin"
                ? `<td style="mso-number-format:'\\@'">${getBaseName(
                    entry.baseId
                  )}</td>`
                : ""
            }
            <td style="mso-number-format:'\\@'">${formatDate(
              entry.entryDate
            )}</td>
            <td style="mso-number-format:'\\@'">${entry.entryTime}</td>
            <td class="status-cell" style="${statusClass}; mso-number-format:'\\@'">${getStatusDisplay(
          entry.status
        )}</td>
          </tr>
        `;
      ...
      tableHtml += `
          <tr>
            <td colspan="${
              admin?.role === "generalAdmin" ? 8 : 7
            }" class="footer">סה"כ רשומות: ${allEntries.length}</td>
          </tr>
        </table>`;
